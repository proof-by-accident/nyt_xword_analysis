...
clues %<>% filter(year(puzzle_date)>1990)
...
p.pun.year.full = ggplot(clues %>%
  ...
  theme(text=element_text(size=20), title=element_text(size=title.size))
...
p.pun.day.full = ggplot(clues %>%
  ...
  theme(text=element_text(size=20), title=element_text(size=title.size),legend.position='None')
